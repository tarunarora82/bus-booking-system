...
function createBooking($employeeId, $busNumber, $scheduleDate) {
    ...
    foreach ($bookings as $booking) {
        if ($booking['employee_id'] === $employeeId && $booking['schedule_date'] === $scheduleDate) {
...
function cancelBooking($employeeId, $busNumber) {
    ...
    for ($i = 0; $i < count($bookings); $i++) {
        if ($bookings[$i]['employee_id'] === $employeeId && 
            ...
            $bookings[$i]['schedule_date'] === $today &&
...
try {
    switch ($action) {
        ...
        case 'create-booking':
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                $input = json_decode(file_get_contents('php://input'), true);
                $result = createBooking($input['employee_id'], $input['bus_number'], $input['schedule_date']);
                echo json_encode($result);
            } else {
                echo json_encode(['status' => 'error', 'message' => 'POST method required']);
            }
        ...
        default:
            echo json_encode([
                ...
                'available_actions' => [
                    ...
                    'create-booking', 'cancel-booking', 'employee-bookings', 'get-bookings'
                ]
            ]);
            break;
    }
